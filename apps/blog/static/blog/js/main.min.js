...
P;na+=1){var M=H[na],oa=M.split(".")[0],V=M.split(".")[1];oa=k("#"+oa);var D=k('<div class="clipmask"></div>');M=k('<img src="static/blog/img/'+M+'" alt="">');if(V==="png")oa.append(M);else{D.append(M);oa.append(D)}M.bind("load",function(){J+=1;J===P&&setTimeout(function(){y()},1E3)})}},qa=function(y){w=new q;w.onChangeView(function(J){J===t.VIEW.SYSTEM?C("show_exit_ui"):C("hide_exit_ui")});w.run();k("#previous").click(function(){w.previousPlanet()});k("#next").click(function(){w.nextPlanet()});k("#back").click(function(){w.getCurrentView()===
