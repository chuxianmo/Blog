...
class Star(models.Model):
    ...
    @property
    def comments(self):
        return self.star_comments.all().filter(rep_to=None)

    @property
    def comment_user_count(self):
        p = []
        lis = self.star_comments.all()
        for each in lis:
    ...
    def comment_count(self):
        return self.star_comments.count()
...
class StarComment(Comment):  # 星球留言模型
    belong = models.ForeignKey(Star, related_name='star_comments', verbose_name='所属星球')
